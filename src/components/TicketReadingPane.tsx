...
import { newId, removeMany, upsert } from '../data/store';
...
interface TicketReadingPaneProps {
  ticket: Ticket;
  /** Called after a successful persist so the caller can sync its own list/selection state. */
  onUpdate: (ticket: Ticket) => void;
  /** Called after a successful delete — caller decides whether to navigate away or clear a selection. */
  onDeleted: () => void;
  testId?: string;
}

/** Shared ticket body — extracted from TicketDetail so both the standalone /tickets/:id route and the inbox SplitPane can render the same ticket, unchanged. */
export function TicketReadingPane({ ticket, onUpdate, onDeleted, testId }: TicketReadingPaneProps) {
      ...
      <div className="card">
        ...
        <ul className="file-list">
          {ticket.attachments.map((f) => (
            <li key={f.id}>
              <FileText size={14} /> {f.name} <span className="muted">({Math.max(1, Math.round(f.size / 1024))} KB)</span>
            </li>
          ))}
        </ul>
      </div>
